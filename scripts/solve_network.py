...
def add_CCL_constraints(n, config):
    ...
    minCCL = pd.Series(config["electricity"]["CCL_min"])
    ext_i = n.generators.query("p_nom_extendable")
    ext_carrier_i = xr.DataArray(ext_i.carrier.rename_axis("Generator-ext"))
    lhs = p_nom.groupby(ext_carrier_i).sum()
    index = minCCL.index.intersection(lhs.indexes["carrier"])
    rhs = minCCL[index].rename_axis("carrier")
    n.model.add_constraints(lhs >= rhs, name="agg_p_nom_min")


    maxCCL = pd.Series(config["electricity"]["CCL_max"])
    ext_i = n.generators.query("p_nom_extendable")
    ext_carrier_i = xr.DataArray(ext_i.carrier.rename_axis("Generator-ext"))
    lhs = p_nom.groupby(ext_carrier_i).sum()
    index = maxCCL.index.intersection(lhs.indexes["carrier"])
    rhs = maxCCL[index].rename_axis("carrier")
    n.model.add_constraints(lhs <= rhs, name="agg_p_nom_max")
...
if __name__ == "__main__":
    if "snakemake" not in globals():
        ...
        snakemake = mock_snakemake(
            ...
            configfiles="/home/susa/Nuova/pypsa-eur_ohnegas/config/config.yaml",
            simpl="",
            opts="",
            clusters="10",
            ll="v20.0",
            sector_opts="3H-T-H-B-I-A-solar-CCL",
